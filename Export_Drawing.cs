    ...
    public class Export_Drawing : UdonSharpBehaviour
    {
        ...
        public string _Export()
        {
            ...
            foreach (Transform ink in pen.inkPoolSynced)
            {
                ...
                {
                    for (int i = 0; i < mesh_vertices.Length; i++)
                        formatPos += "vn 0 1 0\v"; // default up normal
                }
                else
                {
                ...
                for (int i = 0; i < mesh_tris.Length; i += 3)
                {
                    ...
                    formatPos += $"f {v1}/{v1}/{v1} {v2}/{v2}/{v2} {v3}/{v3}/{v3}\v";
                }

                // // export normals
                // for (int i = 0; i < mesh_vertices.Length; i++)
                //    formatPos += string.Format("vn {0} {1} {2}", mesh_vertices[i].x, mesh_vertices[i].y, mesh_vertices[i].z) + "\v";
                // // export UVs
                // for (int i = 0; i < mesh_vertices.Length; i++)
                //    formatPos += string.Format("vt {0} {1}", mesh_vertices[i].x, mesh_vertices[i].y) + "\v";
